...
export async function incrementView(slug) {
    ...
    // Atomic increment using Supabase rpc or direct update
    // Using a single update with a raw SQL expression for atomic increment
    ...
    // Fallback if RPC doesn't exist: use the old method but it's still better to try RPC first
    if (error) {
        // Fallback: fetch-then-update (non-atomic, but works without custom RPC)
